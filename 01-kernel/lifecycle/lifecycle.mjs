...
const NEXT = new Map([
  ['discover','validate'],['validate','define'],['define','build'],['build','verify'],['verify','release'],['release','operate'],['operate','measure'],['measure','learn'],['learn','discover']
]);

export function nextStage(stage) { if (!NEXT.has(stage)) throw new Error(`Unknown ANAS OS stage: ${stage}`); return NEXT.get(stage); }
...
export function transition(project, to) {
  ...
  if (!canTransition(project.stage, to)) throw new Error(`Invalid lifecycle transition: ${project.stage} -> ${to}`);
