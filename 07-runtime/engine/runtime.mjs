...
function wrapTool(def, run) {
  return { id: def.id, authority: def.authority, mutates: Boolean(def.mutates), run };
}
...
export async function loadRuntime() {
  ...
  const engine = new ExecutionEngine({ agents: new Map(), tools: new Map(), audit: event => appendAuditEvent(createAuditEvent({
    type: event.type,
    actorType: 'runtime',
    actorId: 'anas-runtime',
    objectType: 'execution',
    objectId: event.requestId,
    data: event
  })) });
  ...
  for (const def of toolRegistry.tools ?? []) {
    if (def.id === 'filesystem.read') engine.registerTool(wrapTool(def, async ({ path: filePath }) => ({ path: filePath, content: await fs.readFile(safeRelative(filePath), 'utf8') })));
      ...
      const target = safeRelative(filePath);
...
export async function executeGoal({ plan, runtime = await loadRuntime(), context = {} } = {}) {
  ...
  for (const batch of plan.batches ?? []) {
    for (const task of batch) {
      ...
      if (requiresApproval) {
        results.push({ taskId: task.id, status: 'approval-required', reason: 'Consequential execution must be explicitly approved before a mutating or external action.' });
        continue;
      }
  ...
  return { goal: plan.goal, completed: [...completed], results, status: results.some(r => r.status === 'blocked') ? 'blocked' : results.some(r => r.status === 'approval-required') ? 'awaiting-approval' : 'completed' };
}
